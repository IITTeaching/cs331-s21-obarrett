...
class LinkedList:
    ...
    def cursor_insert(self, value):
        ...
        new node"""
        ### BEGIN SOLUTION
        n = self.Node(value,None,None)
        if(self.cursor.next == None):
            n.prior = self.cursor
        else:
            lastn = self.cursor.next
            lastn.prior = n
            n.next = lastn
            n.prior = self.cursor
        self.cursor.next = n
    ...
    def pop(self, idx=-1):
        ...
        x = 0
        z = self.head.next
        while(x <= idx or idx == -1):
            #print(str(z.val) + "    index   " + str(idx) +  "  x  " + str(x))
            if(x == self.length):
                z.prior.next = None
                #print("it returns")
                return z.val
    ...
    def remove(self, value):
        ...
        x = 0
        z = self.head.next
        if(z.val == value and x == 0):
            self.head = z.next
            z.next.prior = self.head
            self.length -= 1
            return z.val
        while(x < self.length):
            if(z.val == value):
    ...
    def index(self, value, i=0, j=None):
        ...
        v = self.length
        if(j != None):
            v = self._normalize_idx(j)
        for x in range(i,v):
            if(value == self[x]):
                return x
        raise ValueError("its not there")   
        ### END SOLUTION
